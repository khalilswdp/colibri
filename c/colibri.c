...
static int64_t g_pin_ram_bytes;   /* bytes PIN_GB actually placed (cap warning quotes it) */
...
static int ds_stage_entry(Model *m,int L,int eid,int64_t *io_off,int64_t limit,
    ...
    for(int k=0;k<3;k++){
        tw[k]=st_find(&m->S,nm[k]);
        snprintf(qn2,sizeof(qn2),"%s.qs",nm[k]); tq[k]=st_find(&m->S,qn2);
    ...
    for(int k=0;k<3;k++){
        int gs=0;
        int fmt=qt_resolve_fmt(tw[k]->name,OO[k],II[k],tw[k]->nbytes,tq[k]->nbytes,&gs);
...
static int expert_load(Model *m, int layer, int eid, ESlot *s, int fatal, int demand){
     ...
     * call site. */
    if(eid<0 || eid>=m->c.n_experts){           /* poisoned eid: report the caller context
                                                 * instead of dying in a fatal tensor lookup */
...
static int spec_decode(Model *m, int *all, int kv, int n_new, int eos, float *logit,
    ...
    while(emitted<n_new && !done && !g_intr && !g_astop){   /* g_intr: stessa uscita del tetto n_new */
...
static void run_serve(Model *m, const char *snap){
    ...
    while((nr=getline(&line,&cap,stdin))>0){
        ...
        if(!strcmp(line,"\x02MORE")){                /* continua la risposta troncata da NGEN:
            ...
            int prod=0;
            if(cur>0){ prod=spec_decode(m,hist,len,cur,eos,logit,emit_stream,&es,&len); ap_flush(&es.ap,stdout); }
            else free(logit);
...
static void pin_load(Model *m, const char *statspath, double gb){
    ...
    g_pin_ram_bytes=(int64_t)(npin-m->gpu_expert_count)*eb;   /* what PIN_GB actually placed */
    ...
    if(floored || capmax==1){
        ...
        if(floored)
            fprintf(stderr,"[RAM_GB=%.1f%s] WARNING: cap=1 is the floor, projected peak %.1f GB "
                "is %.1f GB OVER the budget (resident %.1f GB + reserve %.1f GB). ",
            ...
            fprintf(stderr,"[RAM_GB=%.1f%s] WARNING: this budget affords exactly one cache "
                "slot (resident %.1f GB + reserve %.1f GB leaves %.1f GB). ",
                ram_gb,auto_b?" auto":"",m->resident_bytes/1e9,slack/1e9,
                (ram_gb*1e9-(double)m->resident_bytes-slack)/1e9);
        fprintf(stderr,"The expert cache holds ONE expert per layer (%d total): +%.1f GB of "
            "budget would buy a second slot (--ram %.1f), and PIN_GB accounts for %.1f GB "
            "of the resident set.\n",
...
int main(int argc, char **argv){
    ...
    { double ram_env = getenv("RAM_GB")?atof(getenv("RAM_GB")):0.0;
      ...
      if(!getenv("PIN") && autopin && hist>=5000){
          ...
          if(pin_gb>=0.5 || g_cuda_vram_fill) pin_load(&m, g_usage_path, pin_gb);
      }
